...
3. Run verification in order, stopping at the first failure and reporting it — do not edit code to force it to pass, that's a separate task:
   - `npm run type-check` (`tsc -b`) — this project's root `tsconfig.json` is a references-only solution config (`"files": []`), so plain `tsc --noEmit` checks nothing and always "passes"; `-b` (project reference build mode) is what actually type-checks the app
...
5. Write a Conventional Commits message:
   ...
   - **Never use `feat` unless the user explicitly asked for a feature commit in this request.** Only `feat` commits are meant to reach `prod` — defaulting to it for anything you inferred as "a new feature" misrepresents what's ready to ship. If in doubt, ask rather than guessing `feat`. When a message has multiple instructions, an "as feat" qualifier applies only to the instruction it's grammatically attached to, not to earlier ones in the same message.
   - If a commit still ends up mislabeled despite this, leave it as-is by default — don't amend and force-push over already-pushed history unless the user explicitly asks for that fix.
